...
import { getCareInstructionById, updateCareInstructionById } from 'apiSdk/care-instructions';
...
function CareInstructionEditPage() {
  ...
  const handleSubmit = async (values: CareInstructionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCareInstructionById(id, values);
      mutate(updated);
      resetForm();
